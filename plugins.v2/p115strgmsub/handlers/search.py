...
class SearchHandler:
    ...
    def __init__(
        ...
        :param search_source_order: 自定义搜索源优先级列表，如 ["pansou", ""]；
                                    为空时使用默认优先级  PanSou
        """
    ...
    def _search_(
        ...
        if not self.__client:
            logger.warning(" 客户端未初始化，跳过查询")
            return []

        if not self.__username or not self.__password:
            logger.warning(" 需要配置用户名和密码")
            return []
        ...
        for keyword in search_keywords:
            logger.info(f"使用  搜索: {keyword} (TMDB ID: {mediainfo.tmdb_id})")
            try:
                results = self.__client.search_resources(keyword)
                if results:
                    logger.info(f" 关键词 '{keyword}' 搜索到 {len(results)} 个结果")
                    return results
                logger.info(f" 关键词 '{keyword}' 无结果，尝试下一个降级关键词")
            except Exception as e:
                logger.error(f" 关键词 '{keyword}' 查询失败: {e}")

        logger.info(f" 所有关键词均未找到资源")
        return []
